...
const PinAudio = () => {
  ...
  const { setAudio,audio } = useContext(ChatContext)
  ...
  const handlePlayBackRate = (e) => {
    e.target.checked ? playbackRateMusic(2, true) : playbackRateMusic(1, false)
  }

  return (
    <div
      className={`py-2 px-4 bg-base-200   items-center  justify-between sticky top-0  ${
        audio ? 'flex' : 'hidden'
      }`}
    >
      <div className="flex items-center gap-2  justify-between w-full">
        ...
        <div className="flex items-center gap-0.5">
          ...
          <button className="btn" onClick={() => setAudio(null)}>
            <IoClose size={21} />
          </button>
        </div>
      </div>
