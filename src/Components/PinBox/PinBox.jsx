...
const PinBox = ({ close }) => {
  ...
  return (
    <div
      className={`py-2 px-4 bg-base-200  flex items-center  justify-between sticky top-0 `}
    >
      ...
      <div>
        {pinMessage?.length > 1 ? (
          ...
          <button
            ...
            onClick={() => {
              setShowAlert(true)
              setSenderID(pinMessage[0]?.senderid)
              setMessageID(pinMessage[0]?.messageid)
              setIsPin({
                mID: pinMessage[0]?.messageid,
                isPin: pinMessage[0]?.isPin,
              })
                 setPinMessage([])
            }}
          >
