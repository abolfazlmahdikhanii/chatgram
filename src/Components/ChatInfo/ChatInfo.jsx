...
const ChatInfo = ({ info, chat, setChatInfo }) => {
    ...
    const filterSharedLink = (dis) => {
        ...
        for (const item of newMessages) {
               ...
               newData.push(item.messageDis.match(/\bhttps?:\/\/\S+/gi))
        ...
        <>
                ...
                >
                    ...
                    <div className="mt-9 px-2.5">
                        ...
                        <section>
                            {/* media */}
                            <div className={` grid-cols-3 gap-1.5   mt-6  overflow-hidden ${tab==="Media"||tab==="Files"?'grid':'hidden'}`}>
                                {type &&
                                    type.map((content) => (
                                        <FileType
                                            key={content.id}
                            ...
                            {/* Links */}
                            <ul className={` grid-cols-1 gap-1.5   mt-4  overflow-hidden ${tab==="Links"?'grid':'hidden'}`}>
                                {link &&
                                    link.map((content) => (
                                      <LinkContent key={content.length+1} link={content}/>
                                    ))}
