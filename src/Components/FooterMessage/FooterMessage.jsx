...
const FooterMessage = ({message,date,status,edited,pin,reaction,messageType,caption}) => {
  ...
  return (
    <>
      {messageType === "file"||messageType?.includes('audio')||messageType === "mp3" || messageType === "text"||reaction||caption ? (
        <div className={` flex items-center gap-1 mt-1 px-3 justify-end chat-footer -mr-4 ${messageType?.includes('img')||messageType?.includes('video')?'w-full':''}`}>
